...
export default class graph {
    ...
    draw(){
        ...
        let tempY;
        let rotate = 0;
        let size = 200; //вывести в defaults и прибавлять + 50, тоже самое и в label
        let centerX = this.ctx.canvas.width/2;
        let centerY = this.ctx.canvas.height/2;
        this.ctx.beginPath();
        this.ctx.moveTo(centerX + Defaults.transferX, centerY + Defaults.transferY);
        ...
        for (let d=0; d <= 4; d += 0.01) {
            ...
            this.ctx.lineTo(x1 + Defaults.transferX,y1 + Defaults.transferY);
        }
        this.ctx.stroke();
        this.ctx.closePath();
